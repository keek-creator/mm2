...
Converted["_Open"].BackgroundColor3 = Color3.fromRGB(15,15,15)
...
do
	...
	-- ─── DESIGN TOKENS ──────────────────────────────────────
	local C = {
		bg        = Color3.fromRGB(8,  9,  14),   -- near-black canvas
		surface   = Color3.fromRGB(14, 16, 24),   -- card surface
		elevated  = Color3.fromRGB(20, 23, 36),   -- raised element
		border    = Color3.fromRGB(40, 45, 70),   -- subtle border
		accent1   = Color3.fromRGB(80, 200, 255),  -- cyan neon
		accent2   = Color3.fromRGB(150, 80, 255),  -- violet neon
		accent3   = Color3.fromRGB(255, 80, 150),  -- rose accent
		text      = Color3.fromRGB(230, 235, 255), -- primary text
		textDim   = Color3.fromRGB(100, 110, 150), -- muted text
		danger    = Color3.fromRGB(255, 65,  90),  -- red actions
		success   = Color3.fromRGB(50,  220, 140), -- green confirm
		slider    = Color3.fromRGB(30,  34,  52),  -- slider track
		toggle_on = Color3.fromRGB(80,  200, 255), -- toggle active
		toggle_off= Color3.fromRGB(35,  40,  62),  -- toggle inactive
	}
	...
	local function nkp(t,n)  return NumberSequenceKeypoint.new(t,n) end
	...
	colorCorrection.TintColor  = rgb(200, 205, 255)
	...
	-- ─── HELPER: UIStroke with gradient ─────────────────────
	local function makeStroke(parent, thickness, c1, c2, rotation)
	...
	-- ─── HELPER: UIGradient ──────────────────────────────────
	local function makeGrad(parent, c1, c2, rotation, offset)
		local g = Instance.new("UIGradient")
		g.Color    = csq(ckp(0, c1), ckp(1, c2))
		g.Rotation = rotation or 135
		if offset then g.Offset = offset end
		g.Parent   = parent
		return g
	end

	-- ─── HELPER: inner glow image ────────────────────────────
	local function makeGlow(parent, color, alpha, zindex)
		local il = Instance.new("ImageLabel")
		il.Image            = "rbxassetid://5028857084"
		il.ScaleType        = Enum.ScaleType.Slice
		il.SliceCenter      = Rect.new(24,24,276,276)
		il.BackgroundTransparency = 1
		il.ImageTransparency = alpha or 0.6
		il.ImageColor3      = color
		il.Size             = UDim2.new(1, 30, 1, 30)
		il.Position         = UDim2.new(0, -15, 0, -15)
		il.ZIndex           = zindex or -1
		il.Parent           = parent
		return il
	end

	-- ─── OPEN BUTTON (floating launcher) ────────────────────
	local btn = Converted["_Open"]
	...
	makeStroke(btn, 1.5, C.accent1, C.accent2, 135)
	makeGlow(btn, C.accent1, 0.5, -1)

	-- pulsing glow animation
	local function pulseBtn()
		while btn and btn.Parent do
			tw(btn, 0.9, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, {TextTransparency = 0.15})
			task.wait(0.9)
			tw(btn, 0.9, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, {TextTransparency = 0})
			task.wait(0.9)
		end
	end
	task.spawn(pulseBtn)
	...
	menu.Size                  = UDim2.new(0, 640, 0, 400)
	menu.Position              = UDim2.new(0.5, 0, 0.05, 0)
	menu.AnchorPoint           = Vector2.new(0.5, 0)

	-- Replace old corner
	if Converted["_UICorner14"] then
		Converted["_UICorner14"].CornerRadius = UDim.new(0, 20)
	end

	-- Replace old stroke
	if Converted["_UIStroke5"] then
		Converted["_UIStroke5"].Thickness = 1.2
		Converted["_UIStroke5"].Color = C.accent1
		if Converted["_UIGradient4"] then
			Converted["_UIGradient4"].Color = csq(
				ckp(0,   C.accent1),
				ckp(0.45, rgb(40,45,70)),
				ckp(0.55, rgb(40,45,70)),
				ckp(1,   C.accent2)
			)
			Converted["_UIGradient4"].Rotation = 135
		end
	end

	-- Kill old red gradient on menu
	if Converted["_UIGradient5"] then
		Converted["_UIGradient5"].Color = csq(
			ckp(0,  C.surface),
			ckp(1,  C.bg)
		)
	...
	makeGlow(menu, C.accent1, 0.72, -2)

	-- Subtle scan-line noise overlay  (purely visual frame)
	local noise = Instance.new("Frame")
	noise.Size               = UDim2.new(1,0,1,0)
	noise.BackgroundTransparency = 0.97
	noise.BackgroundColor3   = C.accent2
	noise.ZIndex             = -1
	...
	hubCredits.TextColor3    = C.textDim
	hubCredits.TextTransparency = 0.5
	hubCredits.TextSize      = 11
	...
	badge.BackgroundColor3   = C.accent1
	badge.BackgroundTransparency = 0.75
	badge.TextColor3         = C.accent1
	...
	badge.ZIndex             = 5
	badge.Parent             = menu
	-- position it near HubName
	badge.Position           = UDim2.new(0.055, 0, 0.056, 0)
	local badgeC = Instance.new("UICorner") badgeC.CornerRadius=UDim.new(0,4) badgeC.Parent=badge
	makeStroke(badge, 1, C.accent1)

	-- ─── DIVIDER LINE under header ───────────────────────────
	local divider = Instance.new("Frame")
	divider.Size               = UDim2.new(0.88, 0, 0, 1)
	divider.Position           = UDim2.new(0.06, 0, 0.22, 0)
	divider.BackgroundColor3   = C.border
	divider.BackgroundTransparency = 0
	divider.BorderSizePixel    = 0
	divider.ZIndex             = 2
	divider.Parent             = menu
	makeGrad(divider, C.accent1, C.accent2, 0)

	-- ─── LIST PANEL (left sidebar) ───────────────────────────
	local list = Converted["_List"]
	list.BackgroundColor3      = C.elevated
	list.BackgroundTransparency = 0.08
	list.Size                  = UDim2.new(0.3, 0, 0.78, 0)
	list.Position              = UDim2.new(0.01, 0, 0.21, 0) -- top-left under header

	if Converted["_UICorner19"] then
		Converted["_UICorner19"].CornerRadius = UDim.new(0, 14)
	end
	makeStroke(list, 1, C.border, C.accent1, 135)

	-- scrollbar style
	if Converted["_ScrollingFrame1"] then
		Converted["_ScrollingFrame1"].ScrollBarThickness = 2
		Converted["_ScrollingFrame1"].ScrollBarImageColor3 = C.accent1
	end

	-- ─── AREA PANEL (main content right) ────────────────────
	local area = Converted["_Area"]
	area.BackgroundColor3      = rgb(10,12,20)
	area.BackgroundTransparency = 0.05
	area.Size                  = UDim2.new(0.67, 0, 0.78, 0)
	area.Position              = UDim2.new(0.32, 0, 0.21, 0)
	area.AnchorPoint           = Vector2.new(0,0)

	if Converted["_UICorner18"] then
		Converted["_UICorner18"].CornerRadius = UDim.new(0, 14)
	end
	makeStroke(area, 1, C.border, C.accent2, 135)

	-- Welcome labels inside Area
	if Converted["_TextLabel7"] then
		Converted["_TextLabel7"].Font          = Enum.Font.GothamBlack
		Converted["_TextLabel7"].TextColor3    = C.text
		Converted["_TextLabel7"].TextScaled    = true
		Converted["_TextLabel7"].TextSize      = 28
		Converted["_TextLabel7"].Text          = "YARHM"
	end
	if Converted["_TextLabel6"] then
		Converted["_TextLabel6"].Font          = Enum.Font.GothamMedium
		Converted["_TextLabel6"].TextColor3    = C.textDim
	...
	lb.BackgroundColor3      = C.elevated
	...
	if Converted["_UICorner3"] then
		Converted["_UICorner3"].CornerRadius = UDim.new(0, 10)
	end
	makeStroke(lb, 1, C.border)

	-- ─── ADD CUSTOM MODULE BUTTON ("+") ──────────────────────
	local acm = Converted["_AddCustomModule1"]
	if acm then
		acm.BackgroundColor3 = C.accent1
		acm.BackgroundTransparency = 0.7
		acm.TextColor3       = C.accent1
		acm.Font             = Enum.Font.GothamBlack
		acm.Size             = UDim2.new(0, 32, 0, 32)
		acm.Position         = UDim2.new(1, -4, 1, -4)
		if Converted["_UICorner20"] then
	...
	if acd then
		acd.BackgroundColor3      = C.surface
		acd.BackgroundTransparency = 0.04
		if Converted["_UICorner10"] then
			Converted["_UICorner10"].CornerRadius = UDim.new(0, 18)
		end
		if Converted["_UIStroke2"] then
			Converted["_UIStroke2"].Thickness = 1.2
			Converted["_UIStroke2"].Color = C.accent1
		end
		if Converted["_UIGradient2"] then
			Converted["_UIGradient2"].Color = csq(ckp(0, C.surface), ckp(1, C.elevated))
			Converted["_UIGradient2"].Rotation = 160
		end
		if Converted["_TextLabel2"] then
			Converted["_TextLabel2"].TextColor3 = C.text
			Converted["_TextLabel2"].Font = Enum.Font.GothamBlack
			Converted["_TextLabel2"].Text = "Add a module"
		end
		if Converted["_TextBox1"] then
			Converted["_TextBox1"].BackgroundColor3 = C.elevated
			Converted["_TextBox1"].TextColor3       = C.text
			Converted["_TextBox1"].PlaceholderColor3 = C.textDim
			Converted["_TextBox1"].Font             = Enum.Font.Gotham
			if Converted["_UICorner11"] then Converted["_UICorner11"].CornerRadius = UDim.new(0, 10) end
		end
		if Converted["_TextLabel3"] then
			Converted["_TextLabel3"].TextColor3 = C.danger
			Converted["_TextLabel3"].Font       = Enum.Font.GothamBold
		end
		-- Add button
		if Converted["_Add"] then
			Converted["_Add"].BackgroundColor3 = C.success
			Converted["_Add"].BackgroundTransparency = 0.6
			Converted["_Add"].TextColor3       = C.success
			Converted["_Add"].Font             = Enum.Font.GothamBold
			if Converted["_UICorner12"] then Converted["_UICorner12"].CornerRadius = UDim.new(0, 8) end
			if Converted["_UIStroke3"] then
				Converted["_UIStroke3"].Color = C.success
			end
		end
		-- Cancel button
		if Converted["_Cancel"] then
			Converted["_Cancel"].BackgroundColor3 = C.danger
			Converted["_Cancel"].BackgroundTransparency = 0.6
			Converted["_Cancel"].TextColor3       = C.danger
			Converted["_Cancel"].Font             = Enum.Font.GothamBold
			if Converted["_UICorner13"] then Converted["_UICorner13"].CornerRadius = UDim.new(0, 8) end
			if Converted["_UIStroke4"] then
				Converted["_UIStroke4"].Color = C.danger
			end
		end
	end

	-- ─── CLOSE AREA (drag-to-minimize handle) ────────────────
	if Converted["_Frame1"] then
		Converted["_Frame1"].BackgroundColor3      = C.accent1
		Converted["_Frame1"].BackgroundTransparency = 0.55
	end
	if Converted["_TextLabel5"] then
		Converted["_TextLabel5"].TextColor3        = C.textDim
		Converted["_TextLabel5"].BackgroundColor3  = C.surface
		Converted["_TextLabel5"].BackgroundTransparency = 0.6
	...
	if Converted["_CanvasGroup"] then
		Converted["_CanvasGroup"].BackgroundColor3 = C.bg
	end
	if Converted["_TextLabel4"] then
		Converted["_TextLabel4"].TextColor3        = C.accent1
	...
	if notif then
		...
		if Converted["_UIStroke8"] then
			Converted["_UIStroke8"].Color     = C.accent1
			Converted["_UIStroke8"].Thickness = 1
		end
		if Converted["_UIGradient7"] then
			Converted["_UIGradient7"].Color = csq(ckp(0, C.elevated), ckp(1, C.surface))
			Converted["_UIGradient7"].Rotation = 135
		end
		if Converted["_TextLabel9"] then
			Converted["_TextLabel9"].TextColor3 = C.text
		...
		leftBar.BackgroundColor3   = C.accent1
		leftBar.BackgroundTransparency = 0
		leftBar.BorderSizePixel    = 0
		leftBar.ZIndex             = 6
		local lbCorner = Instance.new("UICorner") lbCorner.CornerRadius=UDim.new(0,999) lbCorner.Parent=leftBar
		leftBar.Parent = notif
	end

	-- ─── DIALOG ──────────────────────────────────────────────
	local dlg = Converted["_Dialog"]
	if dlg then
		dlg.BackgroundColor3 = C.surface
		if Converted["_UICorner25"] then Converted["_UICorner25"].CornerRadius = UDim.new(0, 16) end
		if Converted["_UIGradient8"] then
			Converted["_UIGradient8"].Color = csq(ckp(0, C.elevated), ckp(1, C.surface))
			Converted["_UIGradient8"].Rotation = 160
		end
		if Converted["_UIStroke10"] then
			Converted["_UIStroke10"].Color     = C.accent1
			Converted["_UIStroke10"].Thickness = 1.2
		end
		if Converted["_DialogTitle"] then
			Converted["_DialogTitle"].TextColor3 = C.text
			Converted["_DialogTitle"].Font       = Enum.Font.GothamBlack
		end
		if Converted["_DialogDesc"] then
			Converted["_DialogDesc"].TextColor3  = C.textDim
			Converted["_DialogDesc"].Font        = Enum.Font.GothamMedium
		end
		-- Option placeholder
		if Converted["_OptionPlaceholder"] then
			Converted["_OptionPlaceholder"].BackgroundColor3 = C.elevated
			Converted["_OptionPlaceholder"].TextColor3       = C.text
			...
			if Converted["_UIStroke11"] then
				Converted["_UIStroke11"].Color     = C.accent1
				Converted["_UIStroke11"].Thickness = 1
			end
		end
	end
	...
	if Converted["_Dropdown"] then
		Converted["_Dropdown"].BackgroundColor3 = C.surface
		if Converted["_UICorner9"] then Converted["_UICorner9"].CornerRadius = UDim.new(0, 12) end
	end
	if Converted["_TextLabel1"] then
		Converted["_TextLabel1"].TextColor3     = C.text
		Converted["_TextLabel1"].Font           = Enum.Font.GothamMedium
	end
	if Converted["_Frame"] then
		Converted["_Frame"].BackgroundColor3    = C.elevated
		Converted["_Frame"].TextColor3          = C.text
		Converted["_Frame"].Font                = Enum.Font.GothamMedium
	end

	-- ─── FLOATING BUTTON SETTING ─────────────────────────────
	local fbtn = Converted["_FloatingButton"]
	if fbtn then
		fbtn.BackgroundColor3       = C.surface
		fbtn.TextColor3             = C.accent1
		fbtn.Font                   = Enum.Font.GothamBold
		if Converted["_UICorner7"] then Converted["_UICorner7"].CornerRadius = UDim.new(0, 12) end
		makeStroke(fbtn, 1, C.accent1, C.accent2, 135)
	end

	-- ─── CONTROL BAR (visibility / lock / exit) ──────────────
	if Converted["_Visibility"] then
		Converted["_Visibility"].BackgroundColor3 = C.elevated
		Converted["_Visibility"].TextColor3       = C.text
		Converted["_Visibility"].Font             = Enum.Font.GothamMedium
		Converted["_Visibility"].BackgroundTransparency = 0.1
		if Converted["_UICorner30"] then Converted["_UICorner30"].CornerRadius = UDim.new(0, 12) end
		makeStroke(Converted["_Visibility"], 1, C.border, C.accent1, 90)
	end
	if Converted["_Lock1"] then
		Converted["_Lock1"].BackgroundColor3      = C.elevated
		Converted["_Lock1"].TextColor3            = C.text
		Converted["_Lock1"].Font                  = Enum.Font.GothamMedium
		Converted["_Lock1"].BackgroundTransparency = 0.1
		if Converted["_UICorner31"] then Converted["_UICorner31"].CornerRadius = UDim.new(0, 12) end
		makeStroke(Converted["_Lock1"], 1, C.border, C.accent2, 90)
	end
	if Converted["_Exit"] then
		Converted["_Exit"].BackgroundColor3       = C.danger
		Converted["_Exit"].BackgroundTransparency = 0.55
		Converted["_Exit"].TextColor3             = C.danger
		Converted["_Exit"].Font                   = Enum.Font.GothamBlack
		if Converted["_UICorner32"] then Converted["_UICorner32"].CornerRadius = UDim.new(0, 12) end
		makeStroke(Converted["_Exit"], 1, C.danger)
	end
	if Converted["_Tip"] then
		Converted["_Tip"].TextColor3 = C.textDim
		Converted["_Tip"].Font       = Enum.Font.Gotham
	end

	-- ─── TEXTBOX PLACEHOLDER ─────────────────────────────────
	if Converted["_TextBox"] then
		Converted["_TextBox"].BackgroundColor3  = C.elevated
		Converted["_TextBox"].TextColor3        = C.text
		Converted["_TextBox"].PlaceholderColor3 = C.textDim
		Converted["_TextBox"].Font              = Enum.Font.Gotham
		if Converted["_UICorner6"] then Converted["_UICorner6"].CornerRadius = UDim.new(0, 10) end
	end
	if Converted["_TextButton"] then
		Converted["_TextButton"].BackgroundColor3 = C.accent1
		Converted["_TextButton"].BackgroundTransparency = 0.7
		Converted["_TextButton"].TextColor3       = C.accent1
	...
	if Converted["_Lock"] then
		Converted["_Lock"].TextColor3 = C.textDim
		Converted["_Lock"].Font       = Enum.Font.GothamBold
	end

	-- ─── RIPPLE ──────────────────────────────────────────────
	if Converted["_Ripple"] then
		Converted["_Ripple"].BackgroundColor3 = C.accent1
		if Converted["_UICorner8"] then Converted["_UICorner8"].CornerRadius = UDim.new(0, 999) end
	end

	-- ─── PLACEHOLDER FRAME ───────────────────────────────────
	if Converted["_Placeholder"] then
		Converted["_Placeholder"].BackgroundColor3 = C.elevated
		if Converted["_UICorner4"] then Converted["_UICorner4"].CornerRadius = UDim.new(0, 12) end
	end
	if Converted["_TextLabel"] then
		Converted["_TextLabel"].TextColor3 = C.textDim
		Converted["_TextLabel"].Font       = Enum.Font.GothamMedium
	end

	-- ─── STUBS (corner filler squares) ───────────────────────
	if Converted["_Stub"] then
		Converted["_Stub"].BackgroundColor3 = C.elevated
	end
	if Converted["_Stub1"] then
		Converted["_Stub1"].BackgroundColor3 = C.elevated
	end

	-- ─── ANIMATED ACCENT STRIPE at menu top ──────────────────
	local stripe = Instance.new("Frame")
	stripe.Size               = UDim2.new(0.6, 0, 0, 2)
	stripe.Position           = UDim2.new(0.2, 0, 0, 0)
	stripe.AnchorPoint        = Vector2.new(0, 0)
	stripe.BackgroundColor3   = C.accent1
	stripe.BackgroundTransparency = 0
	stripe.BorderSizePixel    = 0
	stripe.ZIndex             = 10
	stripe.Parent             = menu
	local stripeCorner = Instance.new("UICorner")
	stripeCorner.CornerRadius = UDim.new(0, 999)
	stripeCorner.Parent       = stripe
	makeGrad(stripe, C.accent1, C.accent2, 0)

	-- animate stripe shimmer
	task.spawn(function()
		while stripe and stripe.Parent do
			tw(stripe, 1.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, {
	...
	-- ─── SPAWN ANIMATION for menu ────────────────────────────
	menu.Size             = UDim2.new(0, 640, 0, 0)
	menu.BackgroundTransparency = 1
	menu.Visible          = false
	-- (the open/close logic in Animator routine will call tw; but we pre-style defaults)
	menu.Visible          = false

	print("[YARHM PREMIUM UI] ✓ Overhaul applied — glass/neon theme active")
end
