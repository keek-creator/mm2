...
do
	...
	local C = {
		...
		elevated   = Color3.fromRGB(20,  23,  36),
		border     = Color3.fromRGB(40,  45,  70),
		accent1    = Color3.fromRGB(80,  200, 255),   -- cyan
		accent2    = Color3.fromRGB(150,  80, 255),   -- violet
		danger     = Color3.fromRGB(255,  65,  90),
		success    = Color3.fromRGB(50,  220, 140),
		text       = Color3.fromRGB(230, 235, 255),
		textDim    = Color3.fromRGB(100, 110, 150),
		slider     = Color3.fromRGB(30,   34,  52),
	}
	...
	-- add UIStroke helper
	local function addStroke(parent, thickness, col)
	...
	-- add UIGradient helper
	local function addGrad(parent, c1, c2, rot)
	...
	-- inner glow helper
	local function addGlow(parent, color, alpha, zi)
		...
		il.Size                   = UDim2.new(1, 40, 1, 40)
		il.Position               = UDim2.new(0, -20, 0, -20)
		il.ZIndex                 = zi or -1
		il.Parent                 = parent
		return il
	end
	...
	addStroke(btn, 1.5, C.accent1)
	addGlow(btn, C.accent1, 0.55, -1)

	-- pulse animation on launcher button text
	task.spawn(function()
		while btn and btn.Parent do
			tw(btn, 1.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, {TextTransparency = 0.2})
			task.wait(1.0)
			tw(btn, 1.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, {TextTransparency = 0})
			task.wait(1.0)
		end
	end)

	-- ─── MAIN MENU FRAME ─────────────────────────────────────
	-- NOTE: Do NOT touch menu.Size here — the spring animator reads it at init.
	-- Only restyle colors / corners / strokes.
	...
	-- update existing stroke & gradient to cyan→violet
	...
	if Converted["_UIGradient5"] then
		Converted["_UIGradient5"].Color    = csq(ckp(0, C.surface), ckp(1, C.bg))
		Converted["_UIGradient5"].Rotation = 160
		Converted["_UIGradient5"].Offset   = Vector2.new(0, 0)
	end

	-- ambient glow behind the menu
	addGlow(menu, C.accent1, 0.74, -2)

	-- subtle dark overlay for glass feel
	do
		local glass = Instance.new("Frame")
		glass.Size                = UDim2.new(1, 0, 1, 0)
		glass.BackgroundColor3    = C.accent2
		glass.BackgroundTransparency = 0.97
		glass.BorderSizePixel     = 0
		glass.ZIndex              = -1
		glass.Parent              = menu
		local gc = Instance.new("UICorner")
		gc.CornerRadius = UDim.new(0, 20)
		gc.Parent = glass
	end

	-- ─── ANIMATED ACCENT STRIPE (top edge) ───────────────────
	do
		local stripe = Instance.new("Frame")
		stripe.Size               = UDim2.new(0.55, 0, 0, 2)
		stripe.Position           = UDim2.new(0.225, 0, 0, 0)
		...
		sc.Parent = stripe
		addGrad(stripe, C.accent1, C.accent2, 0)

		task.spawn(function()
			while stripe and stripe.Parent do
				tw(stripe, 1.9, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, {
	...
	hubName.Text           = [[<font color="rgb(80,200,255)">Keek</font>]]
	...
	hubCredits.TextTransparency = 0.45
	hubCredits.Visible         = true

	-- version badge
	do
		local b = Instance.new("TextLabel")
		b.Size                = UDim2.new(0, 50, 0, 17)
		b.Position            = UDim2.new(0.055, 0, 0.056, 0)
		b.BackgroundColor3    = C.accent1
		b.BackgroundTransparency = 0.78
		b.TextColor3          = C.accent1
	...
	do
		...
		d.BackgroundColor3    = C.accent1
		d.BorderSizePixel     = 0
		d.ZIndex              = 2
		d.Parent              = menu
		addGrad(d, C.accent1, C.accent2, 0)
	end
	...
	if Converted["_UIStroke6"] then
		Converted["_UIStroke6"].Color     = C.border
		Converted["_UIStroke6"].Thickness = 1
	end
	if Converted["_UIGradient6"] then
		Converted["_UIGradient6"].Color    = csq(ckp(0, C.border), ckp(1, C.accent1))
	...
	addStroke(lb, 1, C.border)

	-- ─── ADD CUSTOM MODULE "+" BUTTON ────────────────────────
	local acm1 = Converted["_AddCustomModule1"]
	if acm1 then
		acm1.BackgroundColor3       = C.accent1
		acm1.BackgroundTransparency = 0.7
		acm1.TextColor3             = C.accent1
		...
		if Converted["_UIStroke7"] then
			Converted["_UIStroke7"].Color = C.accent1
		end
	end
	...
	if Converted["_Add"] then
		...
		Converted["_Add"].BackgroundTransparency = 0.65
		...
		if Converted["_UIStroke3"] then Converted["_UIStroke3"].Color = C.success end
	end
	if Converted["_Cancel"] then
		Converted["_Cancel"].BackgroundColor3       = C.danger
		Converted["_Cancel"].BackgroundTransparency = 0.65
		...
		if Converted["_UIStroke4"] then Converted["_UIStroke4"].Color = C.danger end
	end

	-- ─── CLOSE / MINIMIZE HANDLE ─────────────────────────────
	...
	if Converted["_TextLabel5"] then
		...
		Converted["_TextLabel5"].BackgroundTransparency = 0.55
	...
	if notif then
		...
		-- accent left bar inside notification
		local lb2 = Instance.new("Frame")
		lb2.Size               = UDim2.new(0, 3, 1, -10)
		lb2.Position           = UDim2.new(0, 5, 0, 5)
		lb2.BackgroundColor3   = C.accent1
		lb2.BorderSizePixel    = 0
		lb2.ZIndex             = 6
		lb2.Parent             = notif
		local lb2c = Instance.new("UICorner") lb2c.CornerRadius = UDim.new(0, 999) lb2c.Parent = lb2
	end

	-- ─── DIALOG ──────────────────────────────────────────────
	if Converted["_Dialog"] then
		Converted["_Dialog"].BackgroundColor3 = C.surface
	...
	if Converted["_Frame3"] then
		Converted["_Frame3"].BackgroundColor3 = Color3.fromRGB(35, 40, 62)
		if Converted["_UICorner21"] then Converted["_UICorner21"].CornerRadius = UDim.new(0, 999) end
	end
	if Converted["_Toggler"] then
		Converted["_Toggler"].BackgroundColor3 = Color3.fromRGB(200, 210, 255)
	...
	if Converted["_DropdownFrameSample"] then
		Converted["_DropdownFrameSample"].BackgroundColor3 = C.surface
	...
	if Converted["_FloatingButton"] then
		Converted["_FloatingButton"].BackgroundColor3 = C.surface
		Converted["_FloatingButton"].TextColor3       = C.accent1
		...
		if Converted["_UIStroke1"] then
			Converted["_UIStroke1"].Color     = C.accent1
	...
	if Converted["_TextButton"] then
		...
		Converted["_TextButton"].BackgroundTransparency = 0.72
	...
	if Converted["_Stub"]  then Converted["_Stub"].BackgroundColor3  = C.elevated end
	if Converted["_Stub1"] then Converted["_Stub1"].BackgroundColor3 = C.elevated end

	print("[Keek PREMIUM UI v2.0] ✓ Theme applied — opening works normally")
end
