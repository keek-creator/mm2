...
Converted["_Open"].TextSize = 16
Converted["_Open"].TextTransparency = 0
Converted["_Open"].TextWrapped = true
Converted["_Open"].AnchorPoint = Vector2.new(0.5, 0.5)
Converted["_Open"].BackgroundColor3 = Color3.fromRGB(25,25,25)
Converted["_Open"].BackgroundTransparency = 0
Converted["_Open"].BorderColor3 = Color3.fromRGB(0, 0, 0)
Converted["_Open"].BorderSizePixel = 0
Converted["_Open"].Position = UDim2.new(0.88, 0, 0.12, 0)
Converted["_Open"].Selectable = false
Converted["_Open"].Size = UDim2.new(0, 130, 0, 38)
...
Converted["_Sample"].Font = Enum.Font.GothamMedium
...
Converted["_FloatingButton"].Font = Enum.Font.GothamMedium
...
Converted["_FloatingButton"].Position = UDim2.new(0.03, 0, 0.35, 0)
Converted["_FloatingButton"].Size = UDim2.new(0, 42, 0, 75)
...
Converted["_Menu"].BackgroundColor3 = Color3.fromRGB(10,10,10)
...
Converted["_Menu"].Size = UDim2.new(0, 620, 0, 380)
Converted["_Menu"].Name = "Menu"
Converted["_Menu"].Parent = Converted["_YARHM"]


-- ╔══════════════════════════════════════════════════════════╗
-- ║         YARHM  ·  PREMIUM UI OVERHAUL  ·  v2.0          ║
-- ║   Glass-morphism · Neon Accents · Spring Animations      ║
-- ╚══════════════════════════════════════════════════════════╝

-- ╔══════════════════════════════════════════════════════════════╗
-- ║        KEEK  ·  TOTAL UI REDESIGN  ·  v4.0                  ║
-- ║  Solid fills · Neon glows · Bright text · Rich frames        ║
-- ╚══════════════════════════════════════════════════════════════╝
do
	...
	local C = {
		...
		bg0      = Color3.fromRGB(6,   7,  12),   -- deepest void
		bg1      = Color3.fromRGB(11,  13,  22),   -- menu base
		bg2      = Color3.fromRGB(17,  20,  34),   -- panel surface
		bg3      = Color3.fromRGB(24,  28,  46),   -- elevated card
		bg4      = Color3.fromRGB(32,  38,  62),   -- raised element

		-- borders
		bdr      = Color3.fromRGB(48,  56,  90),
		bdrHi    = Color3.fromRGB(70,  85, 130),

		-- accent neons
		nCyan    = Color3.fromRGB(0,   220, 255),  -- primary
		nViolet  = Color3.fromRGB(160,  70, 255),  -- secondary
		nLime    = Color3.fromRGB(50,  245, 140),  -- confirm/add
		nRed     = Color3.fromRGB(255,  50,  70),  -- danger
		nAmber   = Color3.fromRGB(255, 190,  30),  -- warning/lock
		nSky     = Color3.fromRGB(80,  190, 255),  -- visibility
		nPink    = Color3.fromRGB(255,  80, 180),  -- accent alt

		-- text (all bright – never dimmer than 180)
		textHi   = Color3.fromRGB(255, 255, 255),  -- headings
		textMd   = Color3.fromRGB(210, 220, 255),  -- body
		textLo   = Color3.fromRGB(160, 175, 220),  -- captions / muted
		textDim  = Color3.fromRGB(120, 135, 180),  -- placeholder
	}

	-- ── HELPERS ─────────────────────────────────────────────────
	local function csq(...) return ColorSequence.new{...} end
	local function ckp(t,c)  return ColorSequenceKeypoint.new(t,c) end
	local function nsp(...) return NumberSequence.new{...} end
	local function nkp(t,v)  return NumberSequenceKeypoint.new(t,v) end

	local function tw(o,t,sty,dir,p)
		TS:Create(o, TweenInfo.new(t, sty or Enum.EasingStyle.Quint,
	...
	local function stroke(p, thick, col, grad1, grad2)
		...
		if grad1 then
			...
			g.Color    = csq(ckp(0, grad1), ckp(1, grad2 or col))
			g.Rotation = 135
			g.Parent   = s
		end
		return s
	...
	local function grad(p, c1, c2, rot, transp1, transp2)
		local g = Instance.new("UIGradient")
		g.Color    = csq(ckp(0, c1), ckp(1, c2))
		g.Rotation = rot or 135
		if transp1 then
			g.Transparency = nsp(nkp(0, transp1), nkp(1, transp2 or transp1))
		end
		g.Parent = p
		return g
	end

	-- Bloom glow using sliced radial image
	local function glow(p, col, alpha, sz, zi)
		local il       = Instance.new("ImageLabel")
		il.Image       = "rbxassetid://5028857084"
		il.ScaleType   = Enum.ScaleType.Slice
		il.SliceCenter = Rect.new(24,24,276,276)
		il.BackgroundTransparency = 1
		il.ImageTransparency      = alpha or 0.5
		il.ImageColor3            = col
		sz = sz or 32
		il.Size     = UDim2.new(1, sz*2, 1, sz*2)
		il.Position = UDim2.new(0, -sz,  0, -sz)
		il.ZIndex   = zi or -1
		il.Parent   = p
		return il
	end

	-- Inset highlight bar (top edge shimmer line inside a frame)
	local function topBar(p, c1, c2)
		local bar = Instance.new("Frame")
		bar.Size             = UDim2.new(0.7, 0, 0, 1)
		bar.Position         = UDim2.new(0.15, 0, 0, 1)
		bar.BackgroundColor3 = c1
		bar.BorderSizePixel  = 0
		bar.ZIndex           = p.ZIndex + 2
		bar.Parent           = p
		corner(bar, 999)
		grad(bar, c1, c2 or c1, 0)
		return bar
	end

	-- Shimmer sweep on click
	local function shimmer(btn, col)
		local sw = Instance.new("Frame")
		sw.BackgroundColor3       = col
		sw.BackgroundTransparency = 0.55
		sw.BorderSizePixel        = 0
		sw.Size                   = UDim2.new(0.15, 0, 1, 0)
		sw.Position               = UDim2.new(-0.18, 0, 0, 0)
		sw.ZIndex                 = btn.ZIndex + 5
		local sg = Instance.new("UIGradient")
		sg.Transparency = nsp(nkp(0,1), nkp(0.35,0.3), nkp(0.5,0.15), nkp(0.65,0.3), nkp(1,1))
		sg.Rotation = 0
		sg.Parent   = sw
		corner(sw, 3)
		sw.Parent = btn
		tw(sw, 0.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut,
			{ Position = UDim2.new(1.03, 0, 0, 0) })
		task.delay(0.52, function()
			if sw and sw.Parent then sw:Destroy() end
		end)
	end

	-- Full hover + press + shimmer hook
	local function hook(btn, accent, bgBase, bgAlpha, stk, glowEl)
		btn.AutoButtonColor       = false
		btn.BackgroundColor3      = bgBase
		btn.BackgroundTransparency = bgAlpha
		...
		btn.MouseEnter:Connect(function()
			tw(btn, 0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out,
				{ BackgroundTransparency = math.max(0, bgAlpha - 0.12) })
			if stk  then tw(stk,    0.14, nil, nil, { Thickness = stk.Thickness + 0.7 }) end
			if glowEl then tw(glowEl, 0.14, nil, nil, { ImageTransparency = math.max(0.25, (glowEl.ImageTransparency or 0.5) - 0.18) }) end
		end)
		btn.MouseLeave:Connect(function()
			tw(btn, 0.20, Enum.EasingStyle.Quad, Enum.EasingDirection.Out,
				{ BackgroundTransparency = bgAlpha })
			if stk  then tw(stk,    0.20, nil, nil, { Thickness = stk.Thickness - 0.7 }) end
			if glowEl then tw(glowEl, 0.20, nil, nil, { ImageTransparency = glowEl.ImageTransparency + 0.18 }) end
		end)
		btn.MouseButton1Down:Connect(function()
			tw(sc, 0.09, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, { Scale = 0.92 })
		end)
		btn.MouseButton1Up:Connect(function()
			tw(sc, 0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out, { Scale = 1.0 })
			shimmer(btn, accent)
		end)
		return sc
	end
	...
	-- ── POST-PROCESSING ─────────────────────────────────────────
	do
		-- remove old bloom if any
		for _, e in ipairs(Lit:GetChildren()) do
	...
	menu.BackgroundColor3       = C.bg1
	menu.BackgroundTransparency = 0

	if Converted["_UICorner14"] then Converted["_UICorner14"].CornerRadius = UDim.new(0, 20) end

	-- animated border gradient: cyan → violet
	if Converted["_UIStroke5"] then
		Converted["_UIStroke5"].Thickness = 1.5
		Converted["_UIStroke5"].Color     = C.nCyan
		local sg = Instance.new("UIGradient")
		sg.Color    = csq(ckp(0, C.nCyan), ckp(0.5, C.nViolet), ckp(1, C.nCyan))
		sg.Rotation = 0
		sg.Parent   = Converted["_UIStroke5"]
		task.spawn(function()
			local r = 0
			while Converted["_UIStroke5"] and Converted["_UIStroke5"].Parent do
				r = (r + 1) % 360
				sg.Rotation = r
				task.wait(0.03)
			end
	...
	-- background gradient inside menu
	if Converted["_UIGradient5"] then
		Converted["_UIGradient5"].Color    = csq(ckp(0, C.bg2), ckp(1, C.bg0))
		Converted["_UIGradient5"].Rotation = 160
		Converted["_UIGradient5"].Offset   = Vector2.new(0,0)
	end

	-- ambient glow behind menu
	glow(menu, C.nCyan, 0.80, 40, -2)

	-- subtle violet tint layer
	do
		local tint = Instance.new("Frame")
		tint.Size                   = UDim2.new(1,0,1,0)
		tint.BackgroundColor3       = C.nViolet
	...
	-- animated top-edge accent stripe
	do
		local s = Instance.new("Frame")
		s.Size             = UDim2.new(0.45,0,0,2)
		s.Position         = UDim2.new(0.27,0,0,0)
		s.BackgroundColor3 = C.nCyan
		s.BorderSizePixel  = 0
		s.ZIndex           = 12
		s.Parent           = menu
		corner(s, 999)
		grad(s, C.nCyan, C.nViolet, 0)
		glow(s, C.nCyan, 0.4, 8, -1)
		task.spawn(function()
			while s and s.Parent do
				tw(s, 2.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut,
					{ Size=UDim2.new(0.78,0,0,2), Position=UDim2.new(0.11,0,0,0) })
				task.wait(2.2)
				tw(s, 2.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut,
					{ Size=UDim2.new(0.45,0,0,2), Position=UDim2.new(0.27,0,0,0) })
				task.wait(2.2)
			end
		end)
	end
	...
	do
		...
		d.Position         = UDim2.new(0.06,0,0.215,0)
		d.BackgroundColor3 = C.nCyan
		d.BorderSizePixel  = 0
		d.ZIndex           = 4
		d.Parent           = menu
		grad(d, C.nCyan, C.nViolet, 0)
		glow(d, C.nCyan, 0.55, 6, -1)
	end

	-- version badge
	do
		local b = Instance.new("TextLabel")
		b.Size                   = UDim2.new(0, 56, 0, 19)
		b.Position               = UDim2.new(0.054, 0, 0.054, 0)
		b.BackgroundColor3       = C.nCyan
		b.BackgroundTransparency = 0.72
		b.TextColor3             = C.nCyan
		...
		corner(b, 5)
		stroke(b, 1, C.nCyan)
		glow(b, C.nCyan, 0.5, 8, -1)
	end

	-- ── HEADER TEXTLABELS ────────────────────────────────────────
	local hubName = Converted["_HubName"]
	if hubName then
		hubName.Font           = Enum.Font.GothamBlack
		hubName.TextColor3     = C.textHi
		hubName.TextScaled     = true
		hubName.TextXAlignment = Enum.TextXAlignment.Left
		hubName.RichText       = true
		hubName.Text           = [[<font color="rgb(0,220,255)">Keek</font>]]
		hubName.TextTransparency = 0
	end
	local hubDesc = Converted["_HubDesc"]
	if hubDesc then
		hubDesc.Font             = Enum.Font.GothamMedium
		hubDesc.TextColor3       = C.textMd
		hubDesc.TextTransparency = 0
		hubDesc.TextSize         = 13
		hubDesc.TextXAlignment   = Enum.TextXAlignment.Left
		hubDesc.Text             = "keek hub menu"
	end
	local hubCredits = Converted["_HubCredits"]
	if hubCredits then
		hubCredits.Font             = Enum.Font.Gotham
		hubCredits.TextColor3       = C.textLo
		hubCredits.TextTransparency = 0
		hubCredits.Visible          = true
	end

	-- ════════════════════════════════════════════════════════════
	--  LIST PANEL (left sidebar)
	-- ════════════════════════════════════════════════════════════
	local lp = Converted["_List"]
	if lp then
		lp.BackgroundColor3       = C.bg2
		lp.BackgroundTransparency = 0
	end
	if Converted["_UICorner19"] then Converted["_UICorner19"].CornerRadius = UDim.new(0, 14) end
	if Converted["_UIStroke6"]  then
		Converted["_UIStroke6"].Thickness = 1
		Converted["_UIStroke6"].Color     = C.bdr
	end
	if Converted["_UIGradient6"] then
		Converted["_UIGradient6"].Color    = csq(ckp(0, C.bdrHi), ckp(1, C.nCyan))
		Converted["_UIGradient6"].Rotation = 135
	end
	if Converted["_ScrollingFrame1"] then
		Converted["_ScrollingFrame1"].ScrollBarThickness   = 3
		Converted["_ScrollingFrame1"].ScrollBarImageColor3 = C.nCyan
	end

	-- ════════════════════════════════════════════════════════════
	--  AREA PANEL (right content)
	-- ════════════════════════════════════════════════════════════
	local ap = Converted["_Area"]
	if ap then
		ap.BackgroundColor3       = C.bg2
		ap.BackgroundTransparency = 0
	end
	if Converted["_UICorner18"] then Converted["_UICorner18"].CornerRadius = UDim.new(0, 14) end
	stroke(ap, 1, C.bdr)

	if Converted["_TextLabel7"] then
		Converted["_TextLabel7"].Font             = Enum.Font.GothamBlack
		Converted["_TextLabel7"].TextColor3       = C.textHi
		Converted["_TextLabel7"].TextTransparency = 0
		Converted["_TextLabel7"].TextScaled        = true
	end
	if Converted["_TextLabel6"] then
		Converted["_TextLabel6"].Font             = Enum.Font.GothamMedium
		Converted["_TextLabel6"].TextColor3       = C.textMd
		Converted["_TextLabel6"].TextTransparency = 0
		Converted["_TextLabel6"].TextSize         = 13
		Converted["_TextLabel6"].Text             = "Select a module from the left panel."
	end

	-- ════════════════════════════════════════════════════════════
	--  FRAME  (generic UI frame)  — redesigned with inset depth
	-- ════════════════════════════════════════════════════════════
	if Converted["_Frame"] then
		local fr = Converted["_Frame"]
		fr.BackgroundColor3       = C.bg3
		fr.BackgroundTransparency = 0
		fr.TextColor3             = C.textMd
		fr.Font                   = Enum.Font.GothamMedium
		if Converted["_UICorner9"] then Converted["_UICorner9"].CornerRadius = UDim.new(0, 12) end
		stroke(fr, 1, C.bdr, C.bdrHi, C.nCyan)
		grad(fr, C.bg3, C.bg2, 135)
		topBar(fr, C.nCyan, C.nViolet)
	end

	-- ════════════════════════════════════════════════════════════
	--  TEXTLABEL  (generic label) — much brighter
	-- ════════════════════════════════════════════════════════════
	if Converted["_TextLabel"] then
		Converted["_TextLabel"].Font             = Enum.Font.GothamMedium
		Converted["_TextLabel"].TextColor3       = C.textMd
		Converted["_TextLabel"].TextTransparency = 0
	end
	if Converted["_TextLabel1"] then
		Converted["_TextLabel1"].Font             = Enum.Font.GothamMedium
		Converted["_TextLabel1"].TextColor3       = C.textMd
		Converted["_TextLabel1"].TextTransparency = 0
	end
	if Converted["_TextLabel2"] then
		Converted["_TextLabel2"].Font             = Enum.Font.GothamBlack
		Converted["_TextLabel2"].TextColor3       = C.textHi
		Converted["_TextLabel2"].TextTransparency = 0
	end
	if Converted["_TextLabel3"] then
		Converted["_TextLabel3"].Font             = Enum.Font.GothamBold
		Converted["_TextLabel3"].TextColor3       = C.nRed
		Converted["_TextLabel3"].TextTransparency = 0
	end
	if Converted["_TextLabel4"] then
		Converted["_TextLabel4"].Font             = Enum.Font.GothamBlack
		Converted["_TextLabel4"].TextColor3       = C.nCyan
		Converted["_TextLabel4"].TextTransparency = 0
	end
	if Converted["_TextLabel5"] then
		Converted["_TextLabel5"].Font                   = Enum.Font.Gotham
		Converted["_TextLabel5"].TextColor3             = C.textLo
		Converted["_TextLabel5"].TextTransparency       = 0
		Converted["_TextLabel5"].BackgroundColor3       = C.bg3
		Converted["_TextLabel5"].BackgroundTransparency = 0.35
		Converted["_TextLabel5"].Text                   = "Tap here to minimize."
		Converted["_TextLabel5"].TextSize               = 12
		if Converted["_UICorner17"] then Converted["_UICorner17"].CornerRadius = UDim.new(0, 6) end
	end
	if Converted["_TextLabel8"] then
		Converted["_TextLabel8"].Font             = Enum.Font.GothamMedium
		Converted["_TextLabel8"].TextColor3       = C.textMd
		Converted["_TextLabel8"].TextTransparency = 0
	end
	if Converted["_TextLabel9"] then
		Converted["_TextLabel9"].Font             = Enum.Font.GothamMedium
		Converted["_TextLabel9"].TextColor3       = C.textMd
		Converted["_TextLabel9"].TextTransparency = 0
	end
	if Converted["_TextLabel10"] then
		Converted["_TextLabel10"].Font             = Enum.Font.GothamMedium
		Converted["_TextLabel10"].TextColor3       = C.textMd
		Converted["_TextLabel10"].TextTransparency = 0
	end
	if Converted["_DialogTitle"] then
		Converted["_DialogTitle"].Font             = Enum.Font.GothamBlack
		Converted["_DialogTitle"].TextColor3       = C.textHi
		Converted["_DialogTitle"].TextTransparency = 0
	end
	if Converted["_DialogDesc"] then
		Converted["_DialogDesc"].Font             = Enum.Font.GothamMedium
		Converted["_DialogDesc"].TextColor3       = C.textMd
		Converted["_DialogDesc"].TextTransparency = 0
	end
	if Converted["_BallProgress"] then
		Converted["_BallProgress"].Font             = Enum.Font.GothamBold
		Converted["_BallProgress"].TextColor3       = C.nCyan
		Converted["_BallProgress"].TextTransparency = 0
	end
	if Converted["_Tip"] then
		Converted["_Tip"].Font             = Enum.Font.Gotham
		Converted["_Tip"].TextColor3       = C.textLo
		Converted["_Tip"].TextTransparency = 0
	end
	if Converted["_Lock"] then
		Converted["_Lock"].Font             = Enum.Font.GothamBold
		Converted["_Lock"].TextColor3       = C.nAmber
		Converted["_Lock"].TextTransparency = 0
	end

	-- ════════════════════════════════════════════════════════════
	--  TEXTBOX  (input field) — redesigned with focus glow
	-- ════════════════════════════════════════════════════════════
	local function styleTextBox(tb, cornerInst, id)
		if not tb then return end
		tb.BackgroundColor3       = C.bg3
		tb.BackgroundTransparency = 0
		tb.TextColor3             = C.textHi
		tb.TextTransparency       = 0
		tb.PlaceholderColor3      = C.textDim
		tb.Font                   = Enum.Font.Gotham
		tb.ClearTextOnFocus       = false
		if cornerInst then cornerInst.CornerRadius = UDim.new(0, 10) end

		local stk = stroke(tb, 1, C.bdr)
		local glw = glow(tb, C.nCyan, 0.90, 10, -1)
		grad(tb, C.bg4, C.bg3, 135)

		-- top inset shine
		topBar(tb, C.bdrHi, C.nCyan)

		tb.Focused:Connect(function()
			tw(stk, 0.20, nil, nil, { Color = C.nCyan, Thickness = 1.8 })
			tw(glw, 0.20, nil, nil, { ImageTransparency = 0.45 })
			tw(tb,  0.20, nil, nil, { BackgroundColor3  = C.bg4 })
		end)
		tb.FocusLost:Connect(function()
			tw(stk, 0.25, nil, nil, { Color = C.bdr, Thickness = 1 })
			tw(glw, 0.25, nil, nil, { ImageTransparency = 0.90 })
			tw(tb,  0.25, nil, nil, { BackgroundColor3  = C.bg3 })
		end)
	end

	styleTextBox(Converted["_TextBox"],  Converted["_UICorner6"],  "tb")
	styleTextBox(Converted["_TextBox1"], Converted["_UICorner11"], "tb1")

	-- ════════════════════════════════════════════════════════════
	--  IMAGEBUTTON  (Close notification ×)
	-- ════════════════════════════════════════════════════════════
	local closeBtn = Converted["_Close"]
	if closeBtn then
		closeBtn.BackgroundColor3       = C.nRed
		closeBtn.BackgroundTransparency = 0.55
		closeBtn.ImageColor3            = C.textHi
		if Converted["_UICorner24"] then Converted["_UICorner24"].CornerRadius = UDim.new(0, 999) end
		stroke(closeBtn, 1, C.nRed)
		local cg = glow(closeBtn, C.nRed, 0.60, 10, -1)
		closeBtn.AutoButtonColor = false

		local csc = Instance.new("UIScale") csc.Scale = 1 csc.Parent = closeBtn
		closeBtn.MouseEnter:Connect(function()
			tw(closeBtn, 0.14, nil, nil, { BackgroundTransparency = 0.20 })
			tw(cg, 0.14, nil, nil, { ImageTransparency = 0.35 })
		end)
		closeBtn.MouseLeave:Connect(function()
			tw(closeBtn, 0.20, nil, nil, { BackgroundTransparency = 0.55 })
			tw(cg, 0.20, nil, nil, { ImageTransparency = 0.60 })
		end)
		closeBtn.MouseButton1Down:Connect(function()
			tw(csc, 0.09, nil, nil, { Scale = 0.88 })
		end)
		closeBtn.MouseButton1Up:Connect(function()
			tw(csc, 0.22, Enum.EasingStyle.Back, nil, { Scale = 1.0 })
		end)
	end

	-- ════════════════════════════════════════════════════════════
	--  OPEN / LAUNCHER BUTTON  (the "K" pill)
	-- ════════════════════════════════════════════════════════════
	local openBtn = Converted["_Open"]
	openBtn.BackgroundColor3       = C.bg2
	openBtn.BackgroundTransparency = 0
	openBtn.TextColor3             = C.textHi
	...
	grad(openBtn, C.bg3, C.bg2, 135)
	topBar(openBtn, C.nCyan, C.nViolet)
	local openStk = stroke(openBtn, 1.5, C.nCyan, C.nCyan, C.nViolet)
	local openGlw = glow(openBtn, C.nCyan, 0.50, 16, -1)

	-- pulsing stroke
	task.spawn(function()
		while openBtn and openBtn.Parent do
			tw(openStk, 1.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, { Thickness = 2.4 })
			tw(openGlw, 1.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, { ImageTransparency = 0.35 })
			task.wait(1.0)
			tw(openStk, 1.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, { Thickness = 1.5 })
			tw(openGlw, 1.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, { ImageTransparency = 0.50 })
			task.wait(1.0)
		end
	end)
	hook(openBtn, C.nCyan, C.bg2, 0, openStk, openGlw)

	-- ════════════════════════════════════════════════════════════
	--  LIST BUTTON  (sidebar entry) — solid filled card
	-- ════════════════════════════════════════════════════════════
	local lb = Converted["_ListButton"]
	if lb then
		lb.BackgroundColor3       = C.bg3
		lb.BackgroundTransparency = 0
		lb.TextColor3             = C.textMd
		lb.TextTransparency       = 0
		lb.Font                   = Enum.Font.GothamMedium
		lb.TextSize               = 13
		lb.Size                   = UDim2.new(0.96, 0, 0, 38)
		if Converted["_UICorner3"] then Converted["_UICorner3"].CornerRadius = UDim.new(0, 10) end
		grad(lb, C.bg4, C.bg3, 135)
		topBar(lb, C.bdrHi, C.nCyan)
		local lbStk = stroke(lb, 1, C.bdr, C.bdrHi, C.nCyan)
		hook(lb, C.nCyan, C.bg3, 0, lbStk)
	end

	-- ════════════════════════════════════════════════════════════
	--  SAMPLE  (dropdown item template)
	-- ════════════════════════════════════════════════════════════
	if Converted["_Sample"] then
		Converted["_Sample"].BackgroundColor3       = C.bg3
		Converted["_Sample"].BackgroundTransparency = 0
		Converted["_Sample"].TextColor3             = C.textMd
		Converted["_Sample"].TextTransparency       = 0
		Converted["_Sample"].Font                   = Enum.Font.GothamMedium
		if Converted["_UICorner2"] then Converted["_UICorner2"].CornerRadius = UDim.new(0, 8) end
		grad(Converted["_Sample"], C.bg4, C.bg3, 135)
		stroke(Converted["_Sample"], 1, C.bdr)
	end

	-- ════════════════════════════════════════════════════════════
	--  TEXTBUTTON  (generic clickable) — solid filled, neon border
	-- ════════════════════════════════════════════════════════════
	if Converted["_TextButton"] then
		local tb  = Converted["_TextButton"]
		tb.BackgroundColor3       = C.bg3
		tb.BackgroundTransparency = 0
		tb.TextColor3             = C.textHi
		tb.TextTransparency       = 0
		tb.Font                   = Enum.Font.GothamBold
		if Converted["_UICorner5"] then Converted["_UICorner5"].CornerRadius = UDim.new(0, 10) end
		grad(tb, C.bg4, C.bg3, 135)
		topBar(tb, C.nCyan, C.nViolet)
		local tbStk = stroke(tb, 1.2, C.nCyan, C.nCyan, C.nViolet)
		local tbGlw = glow(tb, C.nCyan, 0.60, 14, -1)
		hook(tb, C.nCyan, C.bg3, 0, tbStk, tbGlw)
	end

	-- ════════════════════════════════════════════════════════════
	--  ADD CUSTOM MODULE "+" BUTTON
	-- ════════════════════════════════════════════════════════════
	local acm1 = Converted["_AddCustomModule1"]
	if acm1 then
		acm1.BackgroundColor3       = C.bg3
		acm1.BackgroundTransparency = 0
		acm1.TextColor3             = C.textHi
		acm1.TextTransparency       = 0
		acm1.Font                   = Enum.Font.GothamBlack
		if Converted["_UICorner20"] then Converted["_UICorner20"].CornerRadius = UDim.new(0, 10) end
		grad(acm1, C.bg4, C.bg3, 135)
		topBar(acm1, C.nCyan, C.nViolet)
		if Converted["_UIStroke7"] then
			Converted["_UIStroke7"].Color     = C.nCyan
			Converted["_UIStroke7"].Thickness = 1.2
		end
		local acmGlw = glow(acm1, C.nCyan, 0.60, 14, -1)
		hook(acm1, C.nCyan, C.bg3, 0, Converted["_UIStroke7"], acmGlw)
	end

	-- ════════════════════════════════════════════════════════════
	--  ADD MODULE DIALOG FRAME
	-- ════════════════════════════════════════════════════════════
	local acd = Converted["_AddCustomModule"]
	if acd then
		acd.BackgroundColor3       = C.bg1
		acd.BackgroundTransparency = 0
		if Converted["_UICorner10"] then Converted["_UICorner10"].CornerRadius = UDim.new(0, 18) end
		if Converted["_UIStroke2"] then
			Converted["_UIStroke2"].Color     = C.nCyan
			Converted["_UIStroke2"].Thickness = 1.4
		end
		if Converted["_UIGradient2"] then
			Converted["_UIGradient2"].Color    = csq(ckp(0, C.bg2), ckp(1, C.bg1))
			Converted["_UIGradient2"].Rotation = 160
		end
		if Converted["_UIGradient3"] then
			Converted["_UIGradient3"].Color    = csq(ckp(0, C.bg2), ckp(1, C.bg1))
			Converted["_UIGradient3"].Rotation = 160
		end
		topBar(acd, C.nCyan, C.nViolet)
		glow(acd, C.nCyan, 0.78, 28, -2)
	end

	-- ════════════════════════════════════════════════════════════
	--  ADD BUTTON  (confirm / lime)
	-- ════════════════════════════════════════════════════════════
	if Converted["_Add"] then
		local a = Converted["_Add"]
		a.BackgroundColor3       = C.nLime
		a.BackgroundTransparency = 0.55
		a.TextColor3             = C.textHi
		a.TextTransparency       = 0
		a.Font                   = Enum.Font.GothamBold
		if Converted["_UICorner12"] then Converted["_UICorner12"].CornerRadius = UDim.new(0, 9) end
		grad(a, C.nLime, Color3.fromRGB(30, 180, 100), 135, 0.45, 0.65)
		local addStk = stroke(a, 1.3, C.nLime)
		local addGlw = glow(a, C.nLime, 0.55, 14, -1)
		hook(a, C.nLime, C.nLime, 0.55, addStk, addGlw)
	end

	-- ════════════════════════════════════════════════════════════
	--  CANCEL BUTTON  (red)
	-- ════════════════════════════════════════════════════════════
	if Converted["_Cancel"] then
		local ca = Converted["_Cancel"]
		ca.BackgroundColor3       = C.nRed
		ca.BackgroundTransparency = 0.55
		ca.TextColor3             = C.textHi
		ca.TextTransparency       = 0
		ca.Font                   = Enum.Font.GothamBold
		if Converted["_UICorner13"] then Converted["_UICorner13"].CornerRadius = UDim.new(0, 9) end
		grad(ca, C.nRed, Color3.fromRGB(180, 30, 50), 135, 0.45, 0.65)
		local cancelStk = stroke(ca, 1.3, C.nRed)
		local cancelGlw = glow(ca, C.nRed, 0.55, 14, -1)
		hook(ca, C.nRed, C.nRed, 0.55, cancelStk, cancelGlw)
	end

	-- ════════════════════════════════════════════════════════════
	--  CLOSE AREA drag handle
	-- ════════════════════════════════════════════════════════════
	if Converted["_Frame1"] then
		Converted["_Frame1"].BackgroundColor3       = C.nCyan
		Converted["_Frame1"].BackgroundTransparency = 0.4
		if Converted["_UICorner16"] then Converted["_UICorner16"].CornerRadius = UDim.new(0, 999) end
		glow(Converted["_Frame1"], C.nCyan, 0.55, 8, -1)
		task.spawn(function()
			while Converted["_Frame1"] and Converted["_Frame1"].Parent do
				tw(Converted["_Frame1"], 1.6, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut,
	...
	-- ════════════════════════════════════════════════════════════
	--  OPENER  (transparent overlay — stays invisible)
	-- ════════════════════════════════════════════════════════════
	if Converted["_Opener"] then
		Converted["_Opener"].BackgroundTransparency = 1
	end

	-- ════════════════════════════════════════════════════════════
	--  TOGGLER  (toggle thumb + track)
	-- ════════════════════════════════════════════════════════════
	if Converted["_Frame3"] then
		Converted["_Frame3"].BackgroundColor3       = C.bg4
		Converted["_Frame3"].BackgroundTransparency = 0
		if Converted["_UICorner21"] then Converted["_UICorner21"].CornerRadius = UDim.new(0, 999) end
		stroke(Converted["_Frame3"], 1.2, C.bdr, C.bdrHi, C.nCyan)
	end
	if Converted["_Toggler"] then
		local tog = Converted["_Toggler"]
		tog.BackgroundColor3       = C.textHi
		tog.BackgroundTransparency = 0
		if Converted["_UICorner22"] then Converted["_UICorner22"].CornerRadius = UDim.new(0, 999) end
		local togGlw = glow(tog, C.nCyan, 0.62, 10, -1)
		tog.AutoButtonColor = false
		tog.MouseEnter:Connect(function()
			tw(tog, 0.13, nil, nil, { BackgroundColor3 = C.nCyan })
			tw(togGlw, 0.13, nil, nil, { ImageTransparency = 0.38 })
		end)
		tog.MouseLeave:Connect(function()
			tw(tog, 0.20, nil, nil, { BackgroundColor3 = C.textHi })
			tw(togGlw, 0.20, nil, nil, { ImageTransparency = 0.62 })
		end)
	end
	...
	--  VISIBILITY BUTTON
	-- ════════════════════════════════════════════════════════════
	if Converted["_Visibility"] then
		local v = Converted["_Visibility"]
		v.BackgroundColor3       = C.bg3
		v.BackgroundTransparency = 0
		v.TextColor3             = C.textHi
		v.TextTransparency       = 0
		v.Font                   = Enum.Font.GothamMedium
		if Converted["_UICorner30"] then Converted["_UICorner30"].CornerRadius = UDim.new(0, 12) end
		grad(v, C.bg4, C.bg3, 135)
		topBar(v, C.nSky, C.nCyan)
		local vStk = stroke(v, 1.2, C.nSky, C.nSky, C.nCyan)
		local vGlw = glow(v, C.nSky, 0.58, 14, -1)
		hook(v, C.nSky, C.bg3, 0, vStk, vGlw)
	end

	-- ════════════════════════════════════════════════════════════
	--  LOCK BUTTON  (amber)
	-- ════════════════════════════════════════════════════════════
	if Converted["_Lock1"] then
		local lk = Converted["_Lock1"]
		lk.BackgroundColor3       = C.bg3
		lk.BackgroundTransparency = 0
		lk.TextColor3             = C.textHi
		lk.TextTransparency       = 0
		lk.Font                   = Enum.Font.GothamMedium
		if Converted["_UICorner31"] then Converted["_UICorner31"].CornerRadius = UDim.new(0, 12) end
		grad(lk, C.bg4, C.bg3, 135)
		topBar(lk, C.nAmber, Color3.fromRGB(255,140,20))
		local lkStk = stroke(lk, 1.2, C.nAmber)
		local lkGlw = glow(lk, C.nAmber, 0.58, 14, -1)
		hook(lk, C.nAmber, C.bg3, 0, lkStk, lkGlw)
	end

	-- ════════════════════════════════════════════════════════════
	--  EXIT BUTTON  (full red, prominent)
	-- ════════════════════════════════════════════════════════════
	if Converted["_Exit"] then
		local ex = Converted["_Exit"]
		ex.BackgroundColor3       = C.nRed
		ex.BackgroundTransparency = 0.40
		ex.TextColor3             = C.textHi
		ex.TextTransparency       = 0
		ex.Font                   = Enum.Font.GothamBlack
		if Converted["_UICorner32"] then Converted["_UICorner32"].CornerRadius = UDim.new(0, 12) end
		grad(ex, C.nRed, Color3.fromRGB(180,25,40), 135, 0.30, 0.50)
		topBar(ex, Color3.fromRGB(255,120,130), C.nRed)
		local exStk = stroke(ex, 1.5, C.nRed)
		local exGlw = glow(ex, C.nRed, 0.50, 18, -1)
		hook(ex, C.nRed, C.nRed, 0.40, exStk, exGlw)
	end

	-- ════════════════════════════════════════════════════════════
	--  OPTION PLACEHOLDER  (dialog choice button)
	-- ════════════════════════════════════════════════════════════
	if Converted["_OptionPlaceholder"] then
		local op = Converted["_OptionPlaceholder"]
		op.BackgroundColor3       = C.bg3
		op.BackgroundTransparency = 0
		op.TextColor3             = C.textHi
		op.TextTransparency       = 0
		op.Font                   = Enum.Font.GothamBold
		if Converted["_UICorner26"] then Converted["_UICorner26"].CornerRadius = UDim.new(0, 10) end
		grad(op, C.bg4, C.bg3, 135)
		topBar(op, C.nViolet, C.nCyan)
		local opStk = stroke(op, 1.2, C.nViolet, C.nViolet, C.nCyan)
		local opGlw = glow(op, C.nViolet, 0.60, 14, -1)
		hook(op, C.nViolet, C.bg3, 0, opStk, opGlw)
	end

	-- ════════════════════════════════════════════════════════════
	--  BALL  (slider thumb)
	-- ════════════════════════════════════════════════════════════
	if Converted["_Track"] then
		Converted["_Track"].BackgroundColor3       = C.bg4
		Converted["_Track"].BackgroundTransparency = 0
		if Converted["_UICorner28"] then Converted["_UICorner28"].CornerRadius = UDim.new(0, 999) end
		stroke(Converted["_Track"], 1, C.bdr)
	end
	if Converted["_Ball"] then
		local ball = Converted["_Ball"]
		ball.BackgroundColor3       = C.nCyan
		ball.BackgroundTransparency = 0
		if Converted["_UICorner29"] then Converted["_UICorner29"].CornerRadius = UDim.new(0, 999) end
		local ballGlw = glow(ball, C.nCyan, 0.45, 12, -1)
		ball.AutoButtonColor = false
		ball.MouseEnter:Connect(function()
			tw(ball, 0.13, nil, nil, { BackgroundColor3 = Color3.fromRGB(140, 235, 255) })
			tw(ballGlw, 0.13, nil, nil, { ImageTransparency = 0.28 })
		end)
		ball.MouseLeave:Connect(function()
			tw(ball, 0.20, nil, nil, { BackgroundColor3 = C.nCyan })
			tw(ballGlw, 0.20, nil, nil, { ImageTransparency = 0.45 })
		end)
	end
	if Converted["_TrackProgress"] then
		Converted["_TrackProgress"].BackgroundColor3       = C.nCyan
		Converted["_TrackProgress"].BackgroundTransparency = 0
		Converted["_TrackProgress"].TextColor3             = C.nCyan
		Converted["_TrackProgress"].TextTransparency       = 0
		Converted["_TrackProgress"].Font                   = Enum.Font.GothamBold
		if Converted["_UICorner27"] then Converted["_UICorner27"].CornerRadius = UDim.new(0, 999) end
	end

	-- ════════════════════════════════════════════════════════════
	--  FLOATING BUTTON  (draggable launcher)
	-- ════════════════════════════════════════════════════════════
	if Converted["_FloatingButton"] then
		local fb = Converted["_FloatingButton"]
		fb.BackgroundColor3       = C.bg2
		fb.BackgroundTransparency = 0
		fb.TextColor3             = C.textHi
		fb.TextTransparency       = 0
		fb.Font                   = Enum.Font.GothamBold
		if Converted["_UICorner7"] then Converted["_UICorner7"].CornerRadius = UDim.new(0, 14) end
		grad(fb, C.bg3, C.bg2, 135)
		topBar(fb, C.nCyan, C.nViolet)
		local fbStk = stroke(fb, 1.3, C.nCyan, C.nCyan, C.nViolet)
		local fbGlw = glow(fb, C.nCyan, 0.52, 18, -1)
		hook(fb, C.nCyan, C.bg2, 0, fbStk, fbGlw)
	end

	-- ════════════════════════════════════════════════════════════
	--  DROPDOWN FRAME SAMPLE
	-- ════════════════════════════════════════════════════════════
	if Converted["_DropdownFrameSample"] then
		Converted["_DropdownFrameSample"].BackgroundColor3       = C.bg2
		Converted["_DropdownFrameSample"].BackgroundTransparency = 0
		if Converted["_UICorner1"] then Converted["_UICorner1"].CornerRadius = UDim.new(0, 12) end
		if Converted["_UIStroke"] then
			Converted["_UIStroke"].Color     = C.nCyan
			Converted["_UIStroke"].Thickness = 1
		end
		if Converted["_UIGradient"] then
			Converted["_UIGradient"].Color    = csq(ckp(0, C.bg3), ckp(1, C.bg2))
			Converted["_UIGradient"].Rotation = 135
		end
	end
	...
	if Converted["_Dropdown"] then
		Converted["_Dropdown"].BackgroundColor3       = C.bg2
		Converted["_Dropdown"].BackgroundTransparency = 0
		if Converted["_UICorner9"] then Converted["_UICorner9"].CornerRadius = UDim.new(0, 12) end
		stroke(Converted["_Dropdown"], 1, C.nCyan)
		topBar(Converted["_Dropdown"], C.nCyan, C.nViolet)
	end

	-- ════════════════════════════════════════════════════════════
	--  CANVAS GROUP  (loading overlay)
	-- ════════════════════════════════════════════════════════════
	if Converted["_CanvasGroup"] then
		Converted["_CanvasGroup"].BackgroundColor3 = C.bg0
	end

	-- ════════════════════════════════════════════════════════════
	--  NOTIFICATION SAMPLE
	-- ════════════════════════════════════════════════════════════
	local notif = Converted["_NotificationSample"]
	if notif then
		notif.BackgroundColor3       = C.bg2
		notif.BackgroundTransparency = 0
		notif.Size                   = UDim2.new(0, 320, 0, 56)
		if Converted["_UICorner23"] then Converted["_UICorner23"].CornerRadius = UDim.new(0, 14) end
		if Converted["_UIStroke8"] then
			Converted["_UIStroke8"].Color     = C.nCyan
			Converted["_UIStroke8"].Thickness = 1
		end
		if Converted["_UIGradient7"] then
			Converted["_UIGradient7"].Color    = csq(ckp(0, C.bg3), ckp(1, C.bg2))
			Converted["_UIGradient7"].Rotation = 135
		end
		topBar(notif, C.nCyan, C.nViolet)
		-- accent left bar
		local bar = Instance.new("Frame")
		bar.Size             = UDim2.new(0, 3, 1, -10)
		bar.Position         = UDim2.new(0, 5, 0, 5)
		bar.BackgroundColor3 = C.nCyan
		bar.BorderSizePixel  = 0
		bar.ZIndex           = 6
		bar.Parent           = notif
		corner(bar, 999)
		glow(bar, C.nCyan, 0.50, 6, -1)
	end
	...
	if Converted["_Dialog"] then
		Converted["_Dialog"].BackgroundColor3       = C.bg1
		Converted["_Dialog"].BackgroundTransparency = 0
		if Converted["_UICorner25"] then Converted["_UICorner25"].CornerRadius = UDim.new(0, 16) end
		if Converted["_UIGradient8"] then
			Converted["_UIGradient8"].Color    = csq(ckp(0, C.bg2), ckp(1, C.bg1))
			Converted["_UIGradient8"].Rotation = 160
		end
		if Converted["_UIStroke10"] then
			Converted["_UIStroke10"].Color     = C.nCyan
			Converted["_UIStroke10"].Thickness = 1.4
		end
		topBar(Converted["_Dialog"], C.nCyan, C.nViolet)
		glow(Converted["_Dialog"], C.nCyan, 0.75, 28, -2)
	end

	-- ════════════════════════════════════════════════════════════
	--  RIPPLE  (click-ripple frame)
	-- ════════════════════════════════════════════════════════════
	if Converted["_Ripple"] then
		Converted["_Ripple"].BackgroundColor3 = C.nCyan
		if Converted["_UICorner8"] then Converted["_UICorner8"].CornerRadius = UDim.new(0, 999) end
	end

	-- ════════════════════════════════════════════════════════════
	--  PLACEHOLDER / STUBS
	-- ════════════════════════════════════════════════════════════
	if Converted["_Placeholder"] then
		Converted["_Placeholder"].BackgroundColor3       = C.bg3
		Converted["_Placeholder"].BackgroundTransparency = 0
		if Converted["_UICorner4"] then Converted["_UICorner4"].CornerRadius = UDim.new(0, 12) end
		stroke(Converted["_Placeholder"], 1, C.bdr)
	end
	if Converted["_Stub"]  then
		Converted["_Stub"].BackgroundColor3       = C.bg3
		Converted["_Stub"].BackgroundTransparency = 0
	end
	if Converted["_Stub1"] then
		Converted["_Stub1"].BackgroundColor3       = C.bg3
		Converted["_Stub1"].BackgroundTransparency = 0
	end

	print("[Keek TOTAL UI REDESIGN v4.0] ✓ Brighter text · Solid fills · Neon borders · Glows active")
end




Converted["_UICorner14"].CornerRadius = UDim.new(0, 24)
...
Converted["_HubDesc"].Text = "keek hub menu"
...
Converted["_TextLabel4"].Text = "Keek Hub Menu"
...
Converted["_TextLabel7"].Text = "Keek"
...
Converted["_List"].BackgroundColor3 = Color3.fromRGB(15,15,15)
...
Converted["_DialogTitle"].Font = Enum.Font.GothamMedium
...
local function EXEKBZ_routine() -- Routine: StarterGui.YARHM.Forsaken
	...
	if (module.gameId ~= game.GameId) then
		table.insert(module, {
			Type = "Text",
			Args = {"Keek now includes an official <font color='#FFFF00'>Forsaken</font> module!"}
		})
		table.insert(module, {
			Type = "Text",
...
local function XXZOB_routine() -- Routine: StarterGui.YARHM.Murder Mystery 2
	...
	local function getPredictedPosition(player, shootOffset)
		...
		elseif predictionAIEngine and not getgenv().YARHMNetwork.predictPos then
			fu.notification("Keek AI Engine is not available. Reverting to basic prediction.")	
		end
